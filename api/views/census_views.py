...
class CensusUpdate(ModelViewSet):
    ...
    def put(self, request):  # noqa
        ...
        serializer = self.serializer_class(
            data=data, instance=self.queryset.get(pk=data["id"])
        )
        logger.info(
            f"{request.method} - {request.path} - {data} - {request.META['REMOTE_ADDR']} - data_have - "
            f"{status.HTTP_200_OK}"
        )
        if serializer.is_valid():
            serializer.save()
            logger.info(
                f"{request.method} - {request.path} - {data} - {request.META['REMOTE_ADDR']} - saving data - "
                f"{status.HTTP_201_CREATED}"
            )

            return Response(serializer.data, status=status.HTTP_201_CREATED)
        logger.error(
            f"{request.method} - {request.path} - {data} - {request.META['REMOTE_ADDR']} - не сохранено - "
...
class CensusView(ModelViewSet):
    ...
    def get(self, request, *args, **kwargs):
        ...
        logger.info(
            f"{request.method} - {request.path} - {request.META['REMOTE_ADDR']} - {status.HTTP_200_OK}"
        )
...
class AddressesCountView(ModelViewSet):
    ...
    def put(self, request):
        ...
        serializer = self.serializer_class(
            data=data, instance=self.queryset.get(pk=data["census_id"])
        )
        logger.info(
            f"{request.method} - {request.path} - {data} - {request.META['REMOTE_ADDR']} - data_have - "
            f"{status.HTTP_200_OK}"
