...
def report_1(request):
    ...
    user_reports = ReportDownloadFile.objects.order_by("-created_date").filter(
        user=usr
    )[:5]
...
def get_report_1(request):
    if request.method == "POST":
        ...
        data = services.ReportDataOnMongoDB().find_document(
            elements={"depart": depart}, multiple=True, limit=int(limit), skip=int(skip)
        )
...
def filter_report_1(request):
    if request.method == "POST":
        ...
        data = services.ReportDataOnMongoDB().find_document(
            elements=elements, multiple=True, limit=int(limit), skip=int(skip)
        )
        return JsonResponse({"data": data}, safe=False)
    else:
        return {"method is get"}
